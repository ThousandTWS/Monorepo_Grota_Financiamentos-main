...
- **client**: landing page, captação e onboarding. O login/cadastro chama o BFF do painel logista (`NEXT_PUBLIC_LOGISTA_PANEL_URL`) para que o cookie de sessão httpOnly seja emitido no mesmo domínio do painel.
- **logista**: BFF + UI. A camada `/app/api/auth/*` proxia as rotas da API Spring, criptografa/renova sessões (`packages/auth`) e expõe apenas cookies seguros (`grota.logista.session`). O middleware (`apps/logista/middleware.ts`) valida o escopo/role antes de renderizar qualquer rota protegida.
- **admin**: CRM interno com o mesmo padrão de BFF (rotas `/api/auth/*` + middleware). Após autenticar, consome os serviços de propostas/logistas, WebSocket compartilhado (`packages/realtime-*`) e dashboards.
- **packages/**:
...
- **infraestruture/**: playbooks e IaC para subir os serviços (Dockerfiles, pipelines e scripts).
...
1. Usuário cria conta no **client** → `POST ${NEXT_PUBLIC_LOGISTA_PANEL_URL}/api/auth/register`.
2. Painel **logista** proxia o request para `POST /auth/register` (Spring), aguarda verificação e grava sessão quando o login acontece.
3. Ao ser redirecionado para `/visao-geral`, o middleware do painel valida o cookie e carrega `GET /auth/me`.
4. O painel **admin** repete o mesmo modelo (BFF + sessão cifrada) e garante isolamento por papel (`ADMIN_SESSION_SECRET`).
