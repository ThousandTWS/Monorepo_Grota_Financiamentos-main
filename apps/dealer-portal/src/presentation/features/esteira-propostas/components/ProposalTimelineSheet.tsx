...
import { Button as UiButton } from "@/presentation/ui/button";
...
export function ProposalTimelineSheet({ proposalId }: ProposalTimelineSheetProps) {
  const identity = `logista-timeline-${proposalId}`;
      ...
      if (payload?.proposalId === proposalId && payload.text) {
        ...
        setChatMessages((current) => {
          ...
          return [
            ...
            {
              id: messageId,
              sender: isMine ? "Você (Lojista)" : displayName,
              text: payload.text as string,
              timestamp,
              status: isMine ? "sent" : "received",
              mine: isMine,
  ...
  const handleSendChat = useCallback(() => {
    ...
    if (!text) return;
    const messageId = `dealer-${proposalId}-${Date.now()}`;
  ...
  const presenceLabel =
    onlineCount > 1 ? "Admin online" : "Aguardando admin entrar";
      ...
      <SheetContent className="sm:max-w-5xl">
        <SheetHeader>
          ...
          <p className="text-sm text-slate-500">
            Linha do tempo e comunicação entre Lojista e Admin.
          </p>
        </SheetHeader>
        <div className="mt-4">
          {isLoading ? (
            ...
            <ScrollArea className="h-[70vh] pr-2">
              <div className="grid gap-4 md:grid-cols-[1.1fr_0.9fr]">
                ...
                <div className="space-y-3 rounded-lg border border-slate-200 bg-white p-4 shadow-md">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-semibold text-[#134B73]">Chat com o admin</p>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex justify-end gap-2">
                      <UiButton type="button" variant="outline" onClick={handleClearChat}>
                        Limpar
                      </UiButton>
                      <UiButton type="button" onClick={handleSendChat} disabled={!chatInput.trim()}>
                        Enviar mensagem
                      </UiButton>
                    </div>
                  </div>
